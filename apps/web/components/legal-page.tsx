...
export function LegalPage({ title, description, path, sections, version }: LegalPageProps) {
  ...
  return (
    <main id="conteudo" className="mx-auto max-w-4xl px-4 py-14 sm:px-6 sm:py-20">
      ...
      <nav aria-label={t("Trilha de navegação")} className="mb-8 text-sm text-muted">
        <Link
          href="/"
      ...
      <p className="mt-14 border-t border-line pt-7 text-sm text-muted">
        {t("Canonical desta página:")}{" "}
        <span className="break-all font-mono">{absoluteUrl(href(path))}</span>
      </p>
    </main>
  );
}
