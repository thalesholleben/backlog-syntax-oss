...
export default function ServiceAccountsSettingsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: () => notify("error", "Não foi possível criar o service account agora."),
  });
  ...
  const revokeMutation = useMutation({
    ...
    onError: () => notify("error", "Não foi possível revogar agora."),
  });
  ...
  const tokenMutation = useMutation({
    ...
    onError: () => notify("error", "Não foi possível gerar o token agora."),
  });

  return (
    <section className="space-y-8">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-[-0.02em]">Agentes e tokens</h1>
        <p className="mt-1 max-w-lg text-sm text-muted">
          Um service account é um principal técnico ligado a este workspace, nunca uma pessoa
          disfarçada. Escopos: <code>read</code> ou <code>write</code>. Nenhum service account
          recebe <code>admin</code>.
        </p>
      </div>
      ...
      {query.data ? (
        <ul className="max-w-lg space-y-2">
              ...
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="font-mono text-xs text-muted">
                    Escopos são definidos em cada token
                  </p>
                </div>
          ...
          {query.data.length === 0 ? (
            <p className="text-sm text-muted">Nenhum service account ainda.</p>
          ) : null}
        </ul>
      ) : null}
      ...
      >
        <h2 className="font-bold">Novo service account</h2>
        <Field label="Nome">
          {({ inputId }) => (
            <Input
              id={inputId}
