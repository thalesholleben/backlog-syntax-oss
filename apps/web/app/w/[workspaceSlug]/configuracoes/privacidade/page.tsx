...
export default function PrivacySettingsPage() {
  ...
  const accountExportMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      notify("success", "Exportação da conta baixada.");
    },
    onError: () => notify("error", "Não foi possível exportar a conta agora."),
  });
  ...
  const workspaceExportMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      notify("success", "Exportação do workspace baixada.");
    },
    onError: () => notify("error", "Não foi possível exportar este workspace agora."),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      if (error instanceof ApiError && error.code === "sole_owner_workspace") {
        notify("error", "Transfira a propriedade dos workspaces em que você é o único owner.");
        return;
      }
      if (error instanceof ApiError && error.code === "reauthentication_required") {
        notify("error", "Entre novamente para confirmar uma exclusão sensível.");
        return;
      }
      notify("error", "Não foi possível excluir a conta agora.");
    },
  });
  ...
  return (
    <section className="max-w-lg space-y-8">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-[-0.02em]">Privacidade e LGPD</h1>
      ...
      <div className="rounded-card border border-line bg-surface p-5">
        ...
        <p className="mt-2 text-sm leading-6 text-muted">
          Baixa imediatamente um JSON com seu perfil, metadados das suas sessões e memberships.
          Tokens e segredos nunca entram no arquivo.
        </p>
      ...
      <div className="rounded-card border border-danger bg-surface p-5">
        <h2 className="font-bold text-danger">Excluir minha conta</h2>
        <p className="mt-2 text-sm leading-6 text-muted">
          Ação irreversível. A exclusão é bloqueada se você for o único owner de um workspace ativo.
          Dados dos demais membros nunca são apagados.
        </p>
      ...
      >
        <div className="space-y-4">
          <p className="text-sm leading-6 text-muted">
            Digite <strong className="text-foreground">excluir</strong> e o e-mail da conta. Sua
            sessão precisa ter sido iniciada nos últimos 15 minutos.
          </p>
