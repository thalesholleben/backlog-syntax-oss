...
export default function McpSettingsPage() {
  ...
  return (
    <section className="max-w-lg space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-[-0.02em]">Conexão MCP</h1>
        <p className="mt-1 text-sm text-muted">
          Conecte um agente a este workspace pelo protocolo MCP, sem reaproveitar sua sessão de
          navegador.
        </p>
      </div>
      ...
      <ol className="space-y-4">
        ...
        <li className="rounded-2xl border border-line bg-surface p-4">
          ...
          <p className="mt-1 font-semibold">Aponte o cliente MCP para este endpoint:</p>
        ...
        <li className="rounded-2xl border border-line bg-surface p-4">
          ...
          <p className="mt-1 font-semibold">
            Conclua o login e revise os escopos na tela de consentimento. O cliente usa OAuth com
            PKCE e token restrito ao recurso MCP.
          </p>
        </li>
      </ol>

      <p className="rounded-2xl border border-status-blocked bg-surface p-4 text-sm leading-6">
        Tokens de service account são destinados à API REST e não substituem o OAuth do endpoint
        MCP. Revogue uma autorização no cliente e encerre sessões se suspeitar de vazamento.
      </p>
