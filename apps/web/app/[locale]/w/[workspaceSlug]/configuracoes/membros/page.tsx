...
export default function MembersSettingsPage() {
  ...
  return (
    <section className="space-y-6">
      <div>
        <h1 className="font-display text-2xl font-bold tracking-[-0.02em]">{t("Membros")}</h1>
        <p className="mt-1 text-sm text-muted">{t("Pessoas com acesso a este workspace.")}</p>
      </div>
      ...
      {contextQuery.isLoading ? (
        ...
        <ul className="max-w-sm space-y-2">
          <li className="flex items-center justify-between rounded-2xl border border-line bg-surface p-4">
            <PrincipalBadge subjectType={principal.subjectType} name={t("Você")} />
