...
function ConsentForm() {
  ...
  const scopes = (searchParams.get("scope") ?? "read")
    ...
    .filter(Boolean);
  const clientId = searchParams.get("client_id") ?? t("cliente MCP");
  ...
  async function decide(accept: boolean) {
    ...
    try {
      ...
      if (result.error) {
        setError(t("Não foi possível registrar sua decisão."));
        return;
      }
    ...
    <main className="grid min-h-dvh place-items-center bg-background px-4 py-12">
      <section className="w-full max-w-lg rounded-card border border-line bg-surface p-6 shadow-card sm:p-8">
        ...
        <h1 className="mt-3 font-display text-3xl font-bold tracking-[-0.03em]">
          {t("Autorizar acesso ao backlog")}
        </h1>
        <p className="mt-3 text-sm leading-6 text-muted">
          {t("O cliente")} <strong className="text-foreground">{clientId}</strong>{" "}
          {t("solicitou estes escopos. Autorize apenas clientes que você reconhece.")}
        </p>
...
export default function ConsentPage() {
  ...
  return (
    <Suspense
      fallback={
        <main className="grid min-h-dvh place-items-center bg-background px-4 py-12">
          <p className="text-sm text-muted">{t("Carregando autorização...")}</p>
        </main>
      }
    >
      <ConsentForm />
