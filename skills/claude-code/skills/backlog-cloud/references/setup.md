...
Se o servidor ainda não estiver configurado, no projeto que usará a nuvem:

```bash
claude mcp add --transport http --scope local backlog-cloud https://backlog-api.syntaxlab.com.br/mcp
```

Dentro do Claude Code, abra `/mcp`, selecione `backlog-cloud` e autentique
no navegador com a conta do usuário. Revise os escopos apresentados:
`read` para consulta e `write` para as alterações pedidas.
O escopo `local` limita a configuração à relação entre usuário e projeto.

Para uma configuração de equipe solicitada, a entrada equivalente de
`.mcp.json` no projeto é:
...
Mescle apenas essa entrada se o arquivo já existir. O arquivo não contém
segredo; cada usuário faz seu próprio login. Respeite o consentimento do
Claude Code para servidores de projeto. Não adicione headers de token REST.

Use `/mcp` para estado da conexão e reautenticação. Depois, descubra as tools
reais e siga [mcp.md](mcp.md). A skill é invocada por
`/backlog-cloud <pedido>`; ela também pode ser selecionada automaticamente
quando o projeto já adotou o Backlog Syntax.
...
Fonte: [MCP no Claude Code](https://code.claude.com/docs/en/mcp).
